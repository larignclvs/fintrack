...
def test_servico_financeiro_saldo_com_repo_real(db_session):
    ...
    trepo = TransactionRepository(db_session)
    # SQLAlchemy repository provides list_all(); FinanceService expects `list()`.
    # Add a test-only alias so FinanceService can call `list()` without changing src.
...
def test_integridade_constraint_unico_gera_erro(db_session):
    ...
    with pytest.raises(Exception):
        # second user with same email should raise at commit
...
def test_init_db_idempotente(db_session):
    ...
    # call init_db again
    db_mod.init_db()
    # ensure we can still add a user
    from src.repositories.user_repo import UserRepository
    from src.models.user import User
